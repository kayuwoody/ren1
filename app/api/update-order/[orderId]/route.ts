  ...
  try {
    ...
    if (body.meta_data && Array.isArray(body.meta_data)) {
      for (const m of body.meta_data) {
        if (m.key === 'kitchen_ready' && m.value === 'yes') { updates.push('kitchenReady = ?'); values.push(1); }
        if (m.key === 'out_for_delivery' && m.value === 'yes') { updates.push('outForDelivery = ?'); values.push(1); }
        if (m.key === '_locker_number') { updates.push('lockerNumber = ?'); values.push(m.value); }
        if (m.key === '_pickup_code') { updates.push('pickupCode = ?'); values.push(m.value); }
      }
    }
    ...
    if (body.status === 'processing' && existing.status !== 'processing') {
      try {
        ...
        if (!consumptionResponse.ok) {
          console.error(`⚠️ Failed to record inventory consumption for order ${orderId}`);
        }
